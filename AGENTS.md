...
- For web app execution, use the current Umbra web SDK docs. The v5 web SDK is `@umbra-privacy/sdk@5.0.0-rc.6` plus `snarkjs`; the web prover lives at `@umbra-privacy/sdk/zk-prover`. Do not add React Native Umbra prover/module packages to this web repo.
- Do not mix `@umbra-privacy/web-zk-prover` with the v5 SDK unless the official docs change. That package is from an older SDK generation and conflicts with the current web package layout.
- Umbra SDK endpoints must be raw protocol-compatible proxies when used as `rpcUrl`, `indexerApiEndpoint`, or `relayerApiEndpoint`; app-envelope routes like `/web/umbra/holdings` are for UI data, not SDK clients.
- For shield/unshield failures, verify both layers: Worker raw proxy reachability (`/web/rpc/:network`, `/web/umbra/indexer/:network/:proxyPath`, `/web/umbra/relayer/:network/:proxyPath`) and client wallet signer wiring. Non-custodial shield/unshield still requires wallet transaction signatures; do not move private keys, seeds, or master seed material into the Worker.
- For client-side Umbra transaction failures, inspect the SDK simulation payload before changing UI copy. Preserve `simulationLogs` and `simulationErr`, map common cases like insufficient SOL, insufficient token balance, uninitialized accounts, owner mismatch, and compute exhaustion, and avoid retry loops that hide the first real preflight error.
- When `/web/rpc/:network` returns 400 after wallet approval, inspect the JSON-RPC payload before changing wallet code. Umbra SDK execution can use bounded JSON-RPC batches and helper methods such as `getRecentPrioritizationFees` or `isBlockhashValid`; block unsafe methods like `requestAirdrop`, not normal SDK transaction plumbing.
- When a server-side registration check says a wallet is already registered, do not automatically warm the Umbra master seed on page load. Keep `deferMasterSeedSignature: true` and let master-seed authorization happen only inside a user-initiated Umbra operation; do not persist raw Umbra master seed material in browser storage or Worker state.
