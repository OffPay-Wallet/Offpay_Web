...
- When a server-side registration check says a wallet is already registered, the browser may still need a one-time Umbra master-seed authorization for the current tab. Warm and cache that session per wallet/network, but do not persist raw Umbra master seed material in browser storage or Worker state.
